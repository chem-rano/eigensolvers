...
#path = "/data/madhumitarano/PR39/Eigen/benchmark/D150/3206/L10/states/"
...
#for restart ----------------------------------
# send Krylov vectors of last completed cumulative iteration (Ylist)
# and continue from next Krylov iteration
cumIter = 7 # manually from summary file
status = loadTTNSFromHdf5(f"../saveTNSs/tns_{cumIter}_0.h5")[1]["status"]
uSH = loadTTNSFromHdf5(f"../saveTNSs/tns_{cumIter}_0.h5")[1]["eigencoefficients"]
print("restarting calculation with status",status)
assert status["cumIter"] == cumIter

tnsList = [] # load Krylov vectors [tnsList==Ylist: Lanczos code]
for ivec in range(1000):
    try:
        filename = f"../saveTNSs/tns_{cumIter}_{ivec}.h5"
        tnsList.append(loadTTNSFromHdf5(filename)[0])
    except FileNotFoundError:
        break
assert(len(tnsList) == uSH.shape[1]) # all vectors are included from earlier iteration
assert(isinstance(tnsList,list))
#for restart ----------------------------------
###############
...
optionsLinear = {"nSweep":5, "iterativeLinearSystemOptions":optsCheck,"convTol":1e-12,
...
ev, tnsList, status = inexactLanczosDiagonalization(operator,guess,sigma,L,maxit,
        eConv,checkFitTol=1e-3,convertUnit="cm-1",status=status,restart=True)
print(status)
